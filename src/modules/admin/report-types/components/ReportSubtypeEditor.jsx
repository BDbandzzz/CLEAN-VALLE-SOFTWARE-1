        ...
        {subtypes.map((subtype, index) => (
          <div
            ...
            className={`rounded-lg border border-border bg-background/80 p-4 ${
              subtype.active === false ? 'opacity-70' : ''
            }`}
          >
            <div className="grid gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" onClick={() => onToggle(subtype.id)}>
                  {subtype.active === false ? <RotateCcw className="size-4" /> : <Power className="size-4" />}
                  {subtype.active === false ? 'Activar' : 'Deshabilitar'}
                </Button>
